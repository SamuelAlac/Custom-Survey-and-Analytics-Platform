...
const ResponseView = () => {
  ...
  if (searchQuery) {
    respondents = respondents.filter((respondent: any) => {
      const lowercasedSearchQuery = searchQuery.toLowerCase();
      return (
        respondent?.id.toLowerCase().includes(lowercasedSearchQuery) ||
        respondent?.email.toLowerCase().includes(lowercasedSearchQuery)  
      );
    });
  }
  ...
  if (isLoading) return <ResponseViewSkeleton />;
          ...
          <div className="w-full">
            ...
            <div className="flex flex-col w-full h-100">
              {currentRespondents?.map((result: any, index: number) => (
                <div
                  key={index}
                  className="w-full p-2 grid grid-cols-5 text-center font-semibold h-10 border-b border-b-[#D9D9D9] hover:bg-black/10"
                >
